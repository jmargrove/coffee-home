...
class MainPage extends Component {
  renderDataButton() {
    if (this.refs.latitude && this.refs.longitude) {
      return (
        <RaisedButton
          label="Data"
          href={`/rainfall?lat=${this.props.coords.lat}&lng=${this.props.coords
            .lng}`}
        />
      );
    } else {
  ...
  renderModelButton() {
    if (this.refs.latitude && this.refs.longitude) {
      return (
        <Link
          to={`/user-input?lat=${this.props.coords.lat}&lng=${this.props.coords
            .lng}`}
        >
            ...
            <div className="coord-box">
              <form className="the-coords">
                ...
                <input
                  ref="latitude"
                  className="lat-input"
                  value={Math.round(this.props.coords.lat * 1000) / 1000}
                  readOnly
                />
                ...
                <input
                  ref="longitude"
                  className="lng-input"
                  value={Math.round(this.props.coords.lng * 1000) / 1000}
                  readOnly
                />
              </form>
              <div className="go-buttons">{this.renderModelButton()}</div>
            </div>
          </div>
        </div>
      </MuiThemeProvider>
